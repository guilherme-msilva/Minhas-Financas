...
?>
<?php 
$page_title = ($id > 0 ? 'Editar Conta' : 'Nova Conta') . ' - Minhas Finanças';
include 'header.php'; 
?>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        <div class="flex items-center space-x-4 mb-8">
            <a href="contas.php" class="p-2 bg-white/60 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/10 rounded-full transition-colors text-slate-600 dark:text-gray-300 hover:text-slate-800 dark:hover:text-white border border-gray-200 dark:border-transparent">
        ...
        <div class="bg-white/60 dark:bg-white/10 backdrop-blur-xl border border-gray-200 dark:border-white/20 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.1)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] p-8">
            ...
            <form method="POST" action="conta.php<?php echo $id > 0 ? '?id='.$id : ''; ?>" enctype="multipart/form-data" class="space-y-6">
                <div>
                    <label for="nome" class="block text-sm font-medium text-slate-600 dark:text-gray-300 mb-2">Nome da Conta</label>
                ...
                <div>
                    <label for="saldo_inicial" class="block text-sm font-medium text-slate-600 dark:text-gray-300 mb-2">Saldo Inicial (R$)</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="cor" class="block text-sm font-medium text-slate-600 dark:text-gray-300 mb-2">Cor de Identificação</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-600 dark:text-gray-300 mb-2">Imagem da Conta</label>
                        <input type="file" name="img" accept="image/*" class="w-full px-4 py-3 rounded-xl bg-white/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-800 dark:text-white file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-cyan-100 dark:file:bg-cyan-500/20 file:text-cyan-700 dark:file:text-cyan-400 hover:file:bg-cyan-200 dark:hover:file:bg-cyan-500/30 transition-colors">
                        <?php if ($img): ?>
                            <div class="mt-2 text-sm text-slate-500 dark:text-gray-400 flex items-center space-x-2">
                                <img src="img/<?php echo htmlspecialchars($img); ?>" alt="Imagem atual" class="w-8 h-8 rounded-full object-cover border border-gray-200 dark:border-white/20">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-600 dark:text-gray-300 mb-2">Status da Conta</label>
                        <label class="flex items-center space-x-3 cursor-pointer mt-3">
                            <input type="checkbox" name="status" value="1" <?php echo $status ? 'checked' : ''; ?> 
                                class="w-5 h-5 rounded border-gray-400 text-cyan-500 focus:ring-cyan-500 bg-white dark:bg-white/10 accent-cyan-500">
                            <span class="text-slate-800 dark:text-white font-medium">Conta Ativa</span>
                        </label>
                    </div>
                </div>
